...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    freestyleDeploymentProvider: {
      getStatusForLatestCommit: vi.fn(),
      getTimelineFromCommits: vi.fn(),
    },
    freestyleProjectStore: {
      resolveSourceRepoId: vi.fn(),
    },
...
describe("deployment status/timeline ownership hardening", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("returns 401 from deployment status before touching Freestyle when Supabase auth is missing", async () => {
    ...
    const response = await GET(
      new Request(
        "https://adorable.test/api/deployment-status?repoId=wrapper-repo-id",
      ),
    );
    ...
    expect(mocks.freestyleProjectStore.resolveSourceRepoId).not.toHaveBeenCalled();
  });

  it("returns 403 from deployment status before touching Freestyle when the user does not own the project", async () => {
  ...
  it("keeps the Freestyle AccessContext gate in deployment timeline after Supabase ownership succeeds", async () => {
    const access = {
    ...
    const { GET } = await import("../deployment-timeline/route");
    const response = await GET(
      new Request(
        "https://adorable.test/api/deployment-timeline?repoId=wrapper-repo-id",
      ),
    );
    ...
    expect(mocks.freestyleProjectStore.resolveSourceRepoId).not.toHaveBeenCalled();
    expect(
      mocks.freestyleDeploymentProvider.getTimelineFromCommits,
